...
(* let rec charListToString l acc =
  match l with
  |[] -> acc
  |c :: l' -> charListToString l' (String.concat "" [acc; String.make 1 c]) *)
...
let rec baseExpression () =
  ...
  if checkListValid expression then input
    else (print_string "invalid Expression"; baseExpression ())
...
let rec assignCharToCmd s =
  let input = outputThenRead (String.concat " " ["Which cmd for ";String.make 1 s;" ? y/n:"]) in
  match input with
